...
object CCPaymentJob {
  private def processCCPayment(order: Order): Option[String] = {
    try {
      ...
      if (createdPayment.getState.equals("approved"))
        Some(createdPayment.getId.toString)
      else
        None
    }
  ...
  def processPayment (order: Order): Option[String] = processCCPayment(order)
}
